...
calculate_slopes <- function(data){
  ...
  sample14 <- MO2_models[14, ]
  fit14 <- sample14$model[[1]]
  sample14 <- tidy(sample14$model[[1]])
  sample14 <-  sample14[2, ]
  sample14$r.squared <- signif(summary(fit14)$adj.r.squared,5)
  
  slopes <- as_tibble(rbind(sample1,sample2,sample3,sample4,sample5,sample6,sample7,sample8,sample9,sample10,sample11,sample12,sample13,sample14))
  ...
  tankname <- as_tibble(finalslopes$tank) %>%
    slice(1:8)
  combineblanks <- as_tibble(c("blank","blank","blank","blank","blank","blank"))
